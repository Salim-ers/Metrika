...
/**
 * Valide et normalise le corps de requête (POST complet ou PATCH partiel).
 * Les champs absents sont ignorés ; les enums invalides renvoient une erreur.
 */
function parseProjectBody(body: Record<string, unknown>, partial: boolean): { data?: ProjectInput; error?: string } {
  ...
  try {
    const projects = await prisma.project.findMany({
      ...
      include: {
        ...
        _count: { select: { cctps: true, dpgfs: true, documents: true, issues: true } },
      },
    });
    return NextResponse.json({ projects });
  } catch (e) {
    return NextResponse.json({ error: "Erreur lors du chargement des projets : " + (e as Error).message }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = parseProjectBody(body as Record<string, unknown>, false);
    if (error || !data || !data.name) {
      return NextResponse.json({ error: error ?? "Le nom du projet est requis." }, { status: 400 });
    }
    if (data.clientId) {
    ...
    const project = await prisma.project.create({ data: { ...data, name: data.name } });
    return NextResponse.json({ project });
  } catch (e) {
    return NextResponse.json({ error: "Erreur lors de la création du projet : " + (e as Error).message }, { status: 500 });
  }
}
